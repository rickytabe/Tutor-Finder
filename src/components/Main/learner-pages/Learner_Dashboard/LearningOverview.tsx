...
const focusTimeData = [
  { name: 'Focus', value: 75, fill: '#3b82f6' }
];
...
const currentSessions: Session[] = [
  { course: 'Advance Level Mathematics', tutor: 'Dr. Smith', startDate: new Date(2025, 2, 1), endDate: new Date(2025, 2, 21), completed: true },
  { course: 'Physics with mechaics', tutor: 'Prof. Johnson', startDate: new Date(2025, 2, 5), endDate: new Date(2025, 2, 25), completed: false },
   { course: 'Advance Level Chemistry', tutor: 'Mr. Brown', startDate: new Date(2025, 2, 5), endDate: new Date(2025, 2, 25), completed: false }
];
    ...
    <div className="space-y-6 hide-scrollbar">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {stats.map(({ id, title, value, Icon }) => (
          <div key={id} className="bg-white p-6 rounded-xl shadow-md border border-gray-100">
        ...
        <div className="space-y-6">
                  ...
                  <div key={index} className="p-4 bg-gray-50 rounded-lg">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className={`px-2 py-1 text-sm rounded-full ${
                        daysLeft <= 3 ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600'
                      }`}>
                        {daysLeft}d left
                      </span>
                    </div>
                    <div className="flex justify-between text-sm text-gray-500">
                      <span>Started: {format(session.startDate, 'MMM dd')}</span>
          ...
          <div className="bg-white p-6 rounded-xl shadow-md">
            ...
            <div className="flex flex-wrap gap-2">
              {['5-Day Streak', 'Top 10% Learner', 'Fast Learner', 'Perfect Attendance'].map((badge, index) => (
